...
def analyze_page():
    ...
    # Load the CSV file with top keywords and their frequencies
    ...
    # Filter keywords dataframe based on selected playlist
    playlist_keywords_df = keywords_df[keywords_df['playlist.name'] == selected_playlist]

    # Assuming there's only one row per playlist in the keywords dataframe
    if not playlist_keywords_df.empty:
        ...
        for i in range(1, 11):  # Loop through top 10 keywords
            ...
            if pd.notnull(keyword) and pd.notnull(frequency):  # Check if keyword and frequency are not null
                keywords.append(keyword)
                frequencies.append(frequency)
        
        # Use two columns for layout (Continue from your existing layout)
        col1, col2 = st.columns(2)
    ...
    # Continue with the rest of your existing code for other sections...
